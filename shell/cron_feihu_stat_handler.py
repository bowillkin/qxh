...
if __name__ == '__main__':
    ...
    try:
        ...
        file_path = '/data/stat/feihu/fh_%s.txt'%_yesterday.strftime('%Y%m%d')
        ...
        FROM `order_item` GROUP BY `order_item`.`order_id`) AS order_items ON `order`.order_id=`order_items`.oid  WHERE express_id=3 AND express_number is not null
        AND `order`.delivery_time IS NOT NULL
        AND `order`.delivery_time>='%s 00:00:00' AND `order`.delivery_time<='%s 23:59:59'
INTO OUTFILE '%s'
FIELDS TERMINATED BY ','
LINES TERMINATED BY '\r\n'"""%(_date,_date,file_path)
        conn.execute(sql)
        print 'FEIHU STAT COMPLETE.'
    except Exception,e:
        print 'CRON_FEIHU_STAT_HANDLER happen error:%s'%e
    finally:
        conn.close()